...
class GenerateEigenvalue(nn.Module):
    def __init__(self, batch_size, length, feat_dim, pool_dim, target_size, eigenvalue_drop_prob, homophily: Union[float, None]) -> None:
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if (self.homophily is None) or (self.homophily >= 0.5):
            # low-pass filter
...
class ChsyConv(nn.Module):
    def __init__(self, batch_size, length, feat_dim, 
                 ...
                 mu: int = 3, xi: float = 4.0, 
                 stigma: float = 0.5, heta: int = 2,  
                 homophily: Union[float, None] = 0.8) -> None:
